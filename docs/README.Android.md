...
# Android build guide
This guide has been tested with x86_64 Ubuntu 16.04 (Xenial) and 17.10 (Artful). It is meant to cross-compile Kodi for Android using **[Kodi's unified depends build system](../tools/depends/README.md)**. Please read it in full before you proceed to familiarize yourself with the build procedure.
...
```
sudo apt install autoconf build-essential cmake curl default-jdk gawk git gperf lib32stdc++6 lib32z1 lib32z1-dev libcurl4-openssl-dev unzip zlib1g-dev
```
...
Extract Android SDK:
```
unzip $HOME/Downloads/sdk-tools-linux-3859397.zip -d $HOME/android-tools/android-sdk-linux
```
...
**NOTE:** Since you're using the latest SDK available, filename can change over time. Adapt the `unzip` command accordingly.
