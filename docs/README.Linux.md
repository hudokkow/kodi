...
Some packages may be missing or outdated in older distributions. Notably `crossguid, libfmt, waylandpp and wayland-protocols` are known to be outdated or missing. Fortunately there is an easy way to build individual packages with **[Kodi's unified depends build system](../tools/depends/README.md)**.
