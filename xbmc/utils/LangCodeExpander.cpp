  ...
  if (strCharCode.size() == 3)
  {
    ...
    for (unsigned int index = 0; index < ARRAY_SIZE(LanguageCodes); ++index)
    {
      if (charCode == LanguageCodes[index].iso639_2 ||
         (checkWin32Locales && LanguageCodes[index].win_id != nullptr && charCode == LanguageCodes[index].win_id))
      {
...
const ISO639 LanguageCodes[189] =
{
  ...
  { "zh", "chi", "zho" },
  { "zu", "zul", nullptr },
  { "zv", "und", nullptr }, // Kodi intern mapping for missing "Undetermined" iso639-1 code
  { "zx", "zxx", nullptr }, // Kodi intern mapping for missing "No linguistic content" iso639-1 code
  { "zy", "mis", nullptr }, // Kodi intern mapping for missing "Miscellaneous languages" iso639-1 code
  { "zz", "mul", nullptr }  // Kodi intern mapping for missing "Multiple languages" iso639-1 code
};
