...
void CVideoDatabase::CreateViews()
{
  ...
  std::string tvshowcounts = PrepareSQL("CREATE VIEW tvshowcounts AS SELECT "
                                       ...
                                       "      nullptrIF(COUNT(episode.c12), 0) AS totalCount,"
                                       "      COUNT(files.playCount) AS watchedcount,"
                                       "      nullptrIF(COUNT(DISTINCT(episode.c12)), 0) AS totalSeasons, "
  ...
  try
  {
    ...
    if (idParentPath < 0)
    {
      if (!strDateAdded.empty())
        strSQL=PrepareSQL("insert into path (idPath, strPath, dateAdded) values (nullptr, '%s', '%s')", strPath1.c_str(), strDateAdded.c_str());
      else
        strSQL=PrepareSQL("insert into path (idPath, strPath) values (nullptr, '%s')", strPath1.c_str());
    }
    else
    {
      if (!strDateAdded.empty())
        strSQL=PrepareSQL("insert into path (idPath, strPath, dateAdded, idParentPath) values (nullptr, '%s', '%s', %i)", strPath1.c_str(), strDateAdded.c_str(), idParentPath);
      else
        strSQL=PrepareSQL("insert into path (idPath, strPath, idParentPath) values (nullptr, '%s', %i)", strPath1.c_str(), idParentPath);
    }
    m_pDS->exec(strSQL);
  ...
  try
  {
    ...
    if (idPath > -1)
    {
      ...
      std::string strSQL = PrepareSQL("SELECT path.useFolderNames, path.scanRecursive, path.noUpdate, path.exclude FROM path WHERE "
                                        "path.idPath = %i AND "
                                        "path.strContent IS NOT nullptr AND path.strContent != '' AND "
                                        "path.strScraper IS NOT nullptr AND path.strScraper != ''", idPath);
...
int CVideoDatabase::AddFile(const std::string& strFileNameAndPath)
{
  ...
  try
  {
    ...
    strSQL=PrepareSQL("insert into files (idFile, idPath, strFileName) values(nullptr, %i, '%s')", idPath, strFileName.c_str());
    m_pDS->exec(strSQL);
...
int CVideoDatabase::AddMovie(const std::string& strFilenameAndPath)
{
  try
  {
    ...
    if (idMovie < 0)
    {
      ...
      std::string strSQL=PrepareSQL("insert into movie (idMovie, idFile) values (nullptr, %i)", idFile);
      m_pDS->exec(strSQL);
...
{
  if (ExecuteQuery("INSERT INTO tvshow(idShow) VALUES(nullptr)"))
...
int CVideoDatabase::AddEpisode(int idShow, const std::string& strFilenameAndPath)
{
  try
  {
    ...
    std::string strSQL=PrepareSQL("insert into episode (idEpisode, idFile, idShow) values (nullptr, %i, %i)", idFile, idShow);
    m_pDS->exec(strSQL);
    return (int)m_pDS->lastinsertid();
  }
  catch (...)
  ...
  try
  {
    ...
    if (idMVideo < 0)
    {
      ...
      if (idFile < 0)
        return -1;
      UpdateFileDateAdded(idFile, strFilenameAndPath);
      std::string strSQL=PrepareSQL("insert into musicvideo (idMVideo, idFile) values (nullptr, %i)", idFile);
      m_pDS->exec(strSQL);
...
int CVideoDatabase::AddToTable(const std::string& table, const std::string& firstField, const std::string& secondField, const std::string& value)
{
  try
  {
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      strSQL = PrepareSQL("insert into %s (%s, %s) values(nullptr, '%s')", table.c_str(), firstField.c_str(), secondField.c_str(), value.substr(0, 255).c_str());
      m_pDS->exec(strSQL);
...
int CVideoDatabase::AddSet(const std::string& strSet, const std::string& strOverview /* = "" */)
{
  ...
  try
  {
    ...
    {
      m_pDS->close();
      strSQL = PrepareSQL("INSERT INTO sets (idSet, strSet, strOverview) VALUES(nullptr, '%s', '%s')", strSet.c_str(), strOverview.c_str());
      m_pDS->exec(strSQL);
...
int CVideoDatabase::AddActor(const std::string& name, const std::string& thumbURLs, const std::string &thumb)
{
  try
  {
    ...
    if (m_pDS->num_rows() == 0)
    {
      m_pDS->close();
      // doesnt exists, add it
      strSQL=PrepareSQL("insert into actor (actor_id, name, art_urls) values(nullptr, '%s', '%s')", trimmedName.substr(0,255).c_str(), thumbURLs.c_str());
      m_pDS->exec(strSQL);
...
void CVideoDatabase::DeleteDetailsForTvShow(int idTvShow)
{
  try
  {
    ...
    for (int iType = VIDEODB_ID_TV_MIN + 1; iType < VIDEODB_ID_TV_MAX; iType++)
      ids.push_back(StringUtils::Format("c%02d=nullptr", iType));
...
//********************************************************************************************************************************
bool CVideoDatabase::GetTvShowInfo(const std::string& strPath, CVideoInfoTag& details, int idTvShow /* = -1 */, CFileItem *item /* = nullptr */)
{
  try
  {
    if (idTvShow < 0)
    ...
    case VIDEODB_TYPE_COUNT:
      {
        ...
        if (value)
          ...
          conditions.push_back(PrepareSQL("c%02d=nullptr", i));
      }
      break;
...
void CVideoDatabase::AddBookMarkToFile(const std::string& strFilenameAndPath, const CBookmark &bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    if (idBookmark >= 0 )
      ...
      strSQL=PrepareSQL("insert into bookmark (idBookmark, idFile, timeInSeconds, totalTimeInSeconds, thumbNailImage, player, playerState, type) values(nullptr,%i,%f,%f,'%s','%s','%s', %i)", idFile, bookmark.timeInSeconds, bookmark.totalTimeInSeconds, bookmark.thumbNailImage.c_str(), bookmark.player.c_str(), bookmark.playerState.c_str(), (int)type);
...
CVideoInfoTag CVideoDatabase::GetDetailsForTvShow(std::unique_ptr<Dataset> &pDS, bool getDetails /* = false */, CFileItem* item /* = nullptr */)
{
  return GetDetailsForTvShow(pDS->get_sql_record(), getDetails, item);
}

CVideoInfoTag CVideoDatabase::GetDetailsForTvShow(const dbiplus::sql_record* const record, bool getDetails /* = false */, CFileItem* item /* = nullptr */)
{
  CVideoInfoTag details;
...
void CVideoDatabase::RemoveContentForPath(const std::string& strPath, CGUIDialogProgress *progress /* = nullptr */)
{
...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  if (iVersion < 82)
  {
    ...
    m_pDS->exec("UPDATE tvshow SET c16=nullptr,c17=nullptr");
  }
  if (iVersion < 83)
  {
...
void CVideoDatabase::SetPlayCount(const CFileItem &item, int count, const CDateTime &date)
{
  ...
  try
  {
    ...
    {
      if (!date.IsValid())
        strSQL = PrepareSQL("update files set playCount=nullptr,lastPlayed=nullptr where idFile=%i", id);
      else
        strSQL = PrepareSQL("update files set playCount=nullptr,lastPlayed='%s' where idFile=%i", date.GetAsDBDateTime().c_str(), id);
    }
...
void CVideoDatabase::CleanDatabase(CGUIDialogProgressBarHandle* handle, const std::set<int>& paths, bool showProgress)
{
  ...
  try
  {
    ...
    sql = "SELECT path.idPath, path.strPath, path.idParentPath FROM path "
            "WHERE NOT ((strContent IS nullptr OR strContent = '') "
                   "AND (strSettings IS nullptr OR strSettings = '') "
                   "AND (strHash IS nullptr OR strHash = '') "
                   "AND (exclude IS nullptr OR exclude != 1))";
    m_pDS->query(sql);
    std::string strIds;
    while (!m_pDS->eof())
    {
    ...
    sql = StringUtils::Format("DELETE FROM path "
                                "WHERE (strContent IS nullptr OR strContent = '') "
                                  "AND (strSettings IS nullptr OR strSettings = '') "
                                  "AND (strHash IS nullptr OR strHash = '') "
                                  "AND (exclude IS nullptr OR exclude != 1) "
                                  "AND (idParentPath IS nullptr OR NOT EXISTS (SELECT 1 FROM (SELECT idPath FROM path) as parentPath WHERE parentPath.idPath = path.idParentPath)) " // MySQL only fix (#5007)
