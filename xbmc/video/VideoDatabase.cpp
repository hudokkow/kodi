...
bool CVideoDatabase::GetSeasonInfo(int idSeason, CVideoInfoTag& details, bool allDetails /* = true */)
{
  ...
  try
  {
    ...
    {
      if (season == 0)
        name = guih->GetLocalizeStrings().Get(20381);
      else
        name = StringUtils::Format(guih->GetLocalizeStrings().Get(20358).c_str(), season);
    }
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
          ...
          g_passwordManager.IsDatabasePathUnlocked(path, *CMediaSourceSettings::GetInstance().GetSources("video"))))
      {
        ...
        {
          if (iSeason == 0)
            strLabel = guih->GetLocalizeStrings().Get(20381);
          else
            strLabel = StringUtils::Format(guih->GetLocalizeStrings().Get(20358).c_str(), iSeason);
        }
        CFileItemPtr pItem(new CFileItem(strLabel));
...
void CVideoDatabase::CleanDatabase(CGUIDialogProgressBarHandle* handle, const std::set<int>& paths, bool showProgress)
{
  ...
  try
  {
    ...
    if (handle)
    {
      handle->SetTitle(guih->GetLocalizeStrings().Get(700));
      handle->SetText("");
    }
    ...
    if (handle)
      handle->SetTitle(guih->GetLocalizeStrings().Get(331));
        ...
        if (sourcePathNotExists)
        {
          ...
          else
          {
            ...
            if (pDialog != NULL)
            {
              ...
              pDialog->SetText(CVariant{StringUtils::Format(guih->GetLocalizeStrings().Get(15013).c_str(), sourceUrl.GetWithoutUserDetails().c_str())});
...
void CVideoDatabase::ExportToXML(const std::string &path, bool singleFile /* = true */, bool images /* = false */, bool actorThumbs /* false */, bool overwrite /*=false*/)
{
  ...
  if (iFailCount > 0)
    HELPERS::ShowOKDialogText(CVariant{647}, CVariant{StringUtils::Format(guih->GetLocalizeStrings().Get(15011).c_str(), iFailCount)});
}
