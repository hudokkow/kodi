...
namespace VIDEO
{
  ...
  bool CVideoInfoScanner::DoScan(const std::string& strDirectory)
  {
    if (m_handle)
    {
      m_handle->SetText(guih->GetLocalizeStrings().Get(20415));
    }
    ...
    if (content == CONTENT_MOVIES ||content == CONTENT_MUSICVIDEOS)
    {
      if (m_handle)
      {
        ...
        m_handle->SetTitle(StringUtils::Format(guih->GetLocalizeStrings().Get(str).c_str(), info->Name().c_str()));
      }
    ...
    {
      if (m_handle)
        m_handle->SetTitle(StringUtils::Format(guih->GetLocalizeStrings().Get(20319).c_str(), info->Name().c_str()));
    ...
    for (int i = 0; i < items.Size(); ++i)
    {
      ...
      else if (ret == INFO_NOT_FOUND)
      {
        ...
        CServiceBroker::GetEventLog().Add(EventPtr(new CMediaLibraryEvent(
          ...
          StringUtils::Format(guih->GetLocalizeStrings().Get(24147).c_str(), mediaType.c_str(), URIUtils::GetFileName(pItem->GetPath()).c_str()),
