...
void CPVREpgContainer::LoadFromDB(void)
{
  ...
  CPVRGUIProgressHandler* progressHandler = new CPVRGUIProgressHandler(guih->GetLocalizeStrings().Get(19250)); // Loading guide from database
...
bool CPVREpgContainer::UpdateEPG(bool bOnlyPending /* = false */)
{
  ...
  if (bShowProgress && !bOnlyPending)
    progressHandler = new CPVRGUIProgressHandler(guih->GetLocalizeStrings().Get(19004)); // Importing guide from clients
