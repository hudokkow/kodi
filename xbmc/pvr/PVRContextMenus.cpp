  ...
  namespace CONTEXTMENUITEM
  {
    ...
    std::string ShowInformation::GetLabel(const CFileItem &item) const
    {
      if (item.GetPVRRecordingInfoTag())
        return guih->GetLocalizeStrings().Get(19053); /* Recording Information */

      return guih->GetLocalizeStrings().Get(19047); /* Programme information */
    }
    ...
    std::string DeleteRecording::GetLabel(const CFileItem &item) const
    {
      ...
      if (recording && recording->IsDeleted())
        return guih->GetLocalizeStrings().Get(19291); /* Delete permanently */

      return guih->GetLocalizeStrings().Get(117); /* Delete */
    }
    ...
    std::string ToggleTimerState::GetLabel(const CFileItem &item) const
    {
      ...
      if (timer && timer->m_state != PVR_TIMER_STATE_DISABLED)
        return guih->GetLocalizeStrings().Get(844); /* Deactivate */

      return guih->GetLocalizeStrings().Get(843); /* Activate */
    }
    ...
    std::string EditTimer::GetLabel(const CFileItem &item) const
    {
      ...
      if (timer)
      {
        ...
        if (timerType && !timerType->IsReadOnly() && timer->GetTimerRuleId() == PVR_TIMER_NO_PARENT)
        {
          const CPVREpgInfoTagPtr epg(item.GetEPGInfoTag());
          if (epg)
            return guih->GetLocalizeStrings().Get(19242); /* Edit timer */
          else
            return guih->GetLocalizeStrings().Get(21450); /* Edit */
        }
      }

      return guih->GetLocalizeStrings().Get(19241); /* View timer information */
    }
    ...
    std::string DeleteTimer::GetLabel(const CFileItem &item) const
    {
      ...
      return guih->GetLocalizeStrings().Get(19060); /* Delete timer */
    }
