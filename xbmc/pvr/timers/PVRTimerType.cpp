...
void CPVRTimerType::InitLifetimeValues(const PVR_TIMER_TYPE &type)
{
  ...
  else if (SupportsLifetime())
  {
    ...
    for (int i = 1; i < 366; ++i)
    {
      m_lifetimeValues.push_back(std::make_pair(StringUtils::Format(guih->GetLocalizeStrings().Get(17999).c_str(), i), i)); // "%s days"
    }
...
void CPVRTimerType::InitPreventDuplicateEpisodesValues(const PVR_TIMER_TYPE &type)
{
  ...
  else if (SupportsRecordOnlyNewEpisodes())
  {
    ...
    m_preventDupEpisodesValues.push_back(std::make_pair(guih->GetLocalizeStrings().Get(815), 0)); // "Record all episodes"
    m_preventDupEpisodesValues.push_back(std::make_pair(guih->GetLocalizeStrings().Get(816), 1)); // "Record only new episodes"
    ...
    for (unsigned int i = 0; i < type.iRecordingGroupSize; ++i)
    {
      ...
      if (strDescr.empty())
      {
        ...
        strDescr = StringUtils::Format("%s %d",
                                       guih->GetLocalizeStrings().Get(811).c_str(), // Recording group
