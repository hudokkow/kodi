...
CPVRTimerInfoTag::CPVRTimerInfoTag(bool bRadio /* = false */) :
  m_strTitle(guih->GetLocalizeStrings().Get(19056)), // New Timer
...
void CPVRTimerInfoTag::UpdateSummary(void)
{
  ...
  if (m_bEndAnyTime)
  {
    m_strSummary = StringUtils::Format("%s %s %s",
        ...
        guih->GetLocalizeStrings().Get(19107).c_str(), // "at"
        m_bStartAnyTime ?
          guih->GetLocalizeStrings().Get(19161).c_str() /* "any time" */ : StartAsLocalTime().GetAsLocalizedTime("", false).c_str());
  }
  else if ((m_iWeekdays != PVR_WEEKDAY_NONE) || (startDate == endDate))
  {
    m_strSummary = StringUtils::Format("%s %s %s %s %s",
        m_iWeekdays != PVR_WEEKDAY_NONE ?
          GetWeekdaysString().c_str() : startDate.c_str(), //for "Any day" set PVR_WEEKDAY_ALLDAYS
        guih->GetLocalizeStrings().Get(19159).c_str(), // "from"
        m_bStartAnyTime ?
          guih->GetLocalizeStrings().Get(19161).c_str() /* "any time" */ : StartAsLocalTime().GetAsLocalizedTime("", false).c_str(),
        guih->GetLocalizeStrings().Get(19160).c_str(), // "to"
        m_bEndAnyTime ?
          guih->GetLocalizeStrings().Get(19161).c_str() /* "any time" */ : EndAsLocalTime().GetAsLocalizedTime("", false).c_str());
  }
  else
  {
...
{
  std::string strReturn = guih->GetLocalizeStrings().Get(305);
  CSingleLock lock(m_critSection);
  if (URIUtils::PathEquals(m_strFileNameAndPath, CPVRTimersPath::PATH_ADDTIMER))
    strReturn = guih->GetLocalizeStrings().Get(19026);
  else if (m_state == PVR_TIMER_STATE_CANCELLED || m_state == PVR_TIMER_STATE_ABORTED)
    strReturn = guih->GetLocalizeStrings().Get(13106);
  else if (m_state == PVR_TIMER_STATE_RECORDING)
    strReturn = guih->GetLocalizeStrings().Get(19162);
  else if (m_state == PVR_TIMER_STATE_CONFLICT_OK)
    strReturn = guih->GetLocalizeStrings().Get(19275);
  else if (m_state == PVR_TIMER_STATE_CONFLICT_NOK)
    strReturn = guih->GetLocalizeStrings().Get(19276);
  else if (m_state == PVR_TIMER_STATE_ERROR)
    strReturn = guih->GetLocalizeStrings().Get(257);
  else if (m_state == PVR_TIMER_STATE_DISABLED)
    strReturn = guih->GetLocalizeStrings().Get(13106);
  else if (m_state == PVR_TIMER_STATE_COMPLETED)
    if (m_bHasChildRecording)
      strReturn = guih->GetLocalizeStrings().Get(19162); // "Recording active"
    else
      strReturn = guih->GetLocalizeStrings().Get(19256); // "Completed"
  else if (m_state == PVR_TIMER_STATE_SCHEDULED || m_state == PVR_TIMER_STATE_NEW)
  {
    if (m_bHasChildRecording)
      strReturn = guih->GetLocalizeStrings().Get(19162); // "Recording active"
    else if (m_bHasChildErrors)
      strReturn = guih->GetLocalizeStrings().Get(257);   // "Error"
    else if (m_bHasChildConflictNOK)
      strReturn = guih->GetLocalizeStrings().Get(19276); // "Conflict error"
    else if (m_iActiveChildTimers > 0)
      strReturn = StringUtils::Format(guih->GetLocalizeStrings().Get(19255).c_str(), m_iActiveChildTimers); // "%d scheduled"
  }
...
void AppendDay(std::string &strReturn, unsigned int iId)
{
  ...
  if (iId > 0)
    strReturn += guih->GetLocalizeStrings().Get(iId).c_str();
  else
    strReturn += "__";
}
} // unnamed namespace
...
std::string CPVRTimerInfoTag::GetWeekdaysString(unsigned int iWeekdays, bool bEpgBased, bool bLongMultiDaysFormat)
{
  ...
  if (iWeekdays == PVR_WEEKDAY_NONE)
    ...
    strReturn = bEpgBased
              ? guih->GetLocalizeStrings().Get(807)  // "Any day"
              : guih->GetLocalizeStrings().Get(808); // "Every day"
  else if (iWeekdays == PVR_WEEKDAY_MONDAY)
    strReturn = guih->GetLocalizeStrings().Get(831); // "Mondays"
  else if (iWeekdays == PVR_WEEKDAY_TUESDAY)
    strReturn = guih->GetLocalizeStrings().Get(832); // "Tuesdays"
  else if (iWeekdays == PVR_WEEKDAY_WEDNESDAY)
    strReturn = guih->GetLocalizeStrings().Get(833); // "Wednesdays"
  else if (iWeekdays == PVR_WEEKDAY_THURSDAY)
    strReturn = guih->GetLocalizeStrings().Get(834); // "Thursdays"
  else if (iWeekdays == PVR_WEEKDAY_FRIDAY)
    strReturn = guih->GetLocalizeStrings().Get(835); // "Fridays"
  else if (iWeekdays == PVR_WEEKDAY_SATURDAY)
    strReturn = guih->GetLocalizeStrings().Get(836); // "Saturdays"
  else if (iWeekdays == PVR_WEEKDAY_SUNDAY)
    strReturn = guih->GetLocalizeStrings().Get(837); // "Sundays"
  else
  {
...
std::string CPVRTimerInfoTag::ChannelName() const
{
  ...
  if (channeltag)
    ...
    strReturn = StringUtils::Format("(%s)", guih->GetLocalizeStrings().Get(809).c_str()); // "Any channel"
...
CPVRTimerInfoTagPtr CPVRTimerInfoTag::CreateInstantTimerTag(const CPVRChannelPtr &channel, int iDuration /* = DEFAULT_PVRRECORD_INSTANTRECORDTIME */)
{
  ...
  newTimer->m_strSummary = StringUtils::Format(guih->GetLocalizeStrings().Get(19093).c_str(), newTimer->Summary().c_str());
...
void CPVRTimerInfoTag::GetNotificationText(std::string &strText) const
{
  ...
  if (stringID != 0)
    strText = StringUtils::Format("%s: '%s'", guih->GetLocalizeStrings().Get(stringID).c_str(), m_strTitle.c_str());
}
...
std::string CPVRTimerInfoTag::GetDeletedNotificationText() const
{
  ...
  return StringUtils::Format("%s: '%s'", guih->GetLocalizeStrings().Get(stringID).c_str(), m_strTitle.c_str());
}
