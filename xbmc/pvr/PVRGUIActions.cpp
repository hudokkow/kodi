...
namespace PVR
{
      ...
      if (m_actions.find(eAction) == m_actions.end())
      {
        switch (eAction)
        {
          case RECORD_INSTANTRECORDTIME:
            m_pDlgSelect->Add(StringUtils::Format(guih->GetLocalizeStrings().Get(19090).c_str(), m_iInstantRecordTime)); // Record next <default duration> minutes
            break;
          case RECORD_30_MINUTES:
            m_pDlgSelect->Add(StringUtils::Format(guih->GetLocalizeStrings().Get(19090).c_str(), 30));  // Record next 30 minutes
            break;
          case RECORD_60_MINUTES:
            m_pDlgSelect->Add(StringUtils::Format(guih->GetLocalizeStrings().Get(19090).c_str(), 60));  // Record next 60 minutes
            break;
          case RECORD_120_MINUTES:
            m_pDlgSelect->Add(StringUtils::Format(guih->GetLocalizeStrings().Get(19090).c_str(), 120)); // Record next 120 minutes
            break;
          case RECORD_CURRENT_SHOW:
            m_pDlgSelect->Add(StringUtils::Format(guih->GetLocalizeStrings().Get(19091).c_str(), title.c_str())); // Record current show (<title>)
            break;
          case RECORD_NEXT_SHOW:
            m_pDlgSelect->Add(StringUtils::Format(guih->GetLocalizeStrings().Get(19092).c_str(), title.c_str())); // Record next show (<title>)
            break;
          case NONE:
          case ASK:
          default:
  ...
  bool CPVRGUIActions::RenameTimer(const CFileItemPtr &item) const
  {
    ...
    if (CGUIKeyboardFactory::ShowAndGetInput(strNewName,
                                             CVariant{guih->GetLocalizeStrings().Get(19042)}, // "Are you sure you want to rename this timer?"
                                             false))
    {
  ...
  bool CPVRGUIActions::RenameRecording(const CFileItemPtr &item) const
  {
    ...
    if (!CGUIKeyboardFactory::ShowAndGetInput(strNewName, CVariant{guih->GetLocalizeStrings().Get(19041)}, false))
      return false;
  ...
  std::string CPVRGUIActions::GetResumeLabel(const CFileItem &item) const
  {
    ...
    if (recording && !recording->IsDeleted())
    {
      ...
      if (positionInSeconds > 0)
        resumeString = StringUtils::Format(guih->GetLocalizeStrings().Get(12022).c_str(),
  ...
  bool CPVRGUIActions::SwitchToChannel(const CFileItemPtr &item, bool bCheckResume) const
  {
    ...
    {
      const std::string channelName = channel ? channel->ChannelName() : guih->GetLocalizeStrings().Get(19029); // Channel
      const std::string msg = StringUtils::Format(guih->GetLocalizeStrings().Get(19035).c_str(), channelName.c_str()); // CHANNELNAME could not be played. Check the log for details.

      CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Error, guih->GetLocalizeStrings().Get(19166), msg); // PVR information
    }
  ...
  bool CPVRGUIActions::SwitchToChannel(PlaybackType type) const
  {
    ...
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Error,
                                          guih->GetLocalizeStrings().Get(19166), // PVR information
                                          StringUtils::Format(guih->GetLocalizeStrings().Get(19035).c_str(),
                                                              guih->GetLocalizeStrings().Get(bIsRadio ? 19021 : 19020).c_str())); // Radio/TV could not be played. Check the log for details.
    return false;
  }
  ...
  bool CPVRGUIActions::ProcessMenuHooks(const CFileItemPtr &item)
  {
    ...
    if (CServiceBroker::GetPVRManager().Clients()->GetCreatedClient(iClientID, client) && client->HasMenuHooks(menuCategory))
    {
      ...
      for (const auto& hook : hooks)
      {
        ...
        {
          pDialog->Add(guih->GetLocalizeStrings().GetAddonString(client->ID(), hook.iLocalizedStringId));
  ...
  bool CPVRGUIActions::ResetPVRDatabase(bool bResetEPGOnly)
  {
    ...
    pDlgProgress->SetLine(0, CVariant{guih->GetLocalizeStrings().Get(19187)}); // "Clearing all related data."
  ...
  ParentalCheckResult CPVRGUIActions::CheckParentalPIN() const
  {
    ...
    InputVerificationResult ret = CGUIDialogNumeric::ShowAndVerifyInput(pinCode, guih->GetLocalizeStrings().Get(19262), true); // "Parental control. Enter PIN:"
      ...
      if (!AllLocalBackendsIdle(cause))
      {
        if (bAskUser)
        {
          ...
          if (cause)
          {
            ...
            {
              text = StringUtils::Format(guih->GetLocalizeStrings().Get(19691).c_str(), // "PVR is currently recording...."
                                         cause->Title().c_str(),
            ...
            else
            {
              ...
              {
                // "%d minutes"
                dueStr = StringUtils::Format(guih->GetLocalizeStrings().Get(19694).c_str(), mins);
              }
              else
              {
                // "about a minute"
                dueStr = guih->GetLocalizeStrings().Get(19695);
              }

              text = StringUtils::Format(guih->GetLocalizeStrings().Get(19692).c_str(), // "PVR will start recording...."
                                         cause->Title().c_str(),
                                         cause->ChannelName().c_str(),
                                         dueStr.c_str());
            }
          ...
          else
          {
            ...
            text = StringUtils::Format(guih->GetLocalizeStrings().Get(19693).c_str(), // "Daily wakeup is due in...."
                                       dueStr.c_str());
          }
