...
std::string CUtil::GetTitleFromPath(const CURL& url, bool bIsFolder /* = false */)
{
  ...
  else if (url.IsProtocol("shout"))
  {
    ...
    if(genre == std::string::npos)
      strFilename = guih->GetLocalizeStrings().Get(260);
    else
      strFilename = guih->GetLocalizeStrings().Get(260) + " - " + strFileNameAndPath.substr(genre+1).c_str();
  }
  ...
  else if (url.IsProtocol("smb") && strFilename.empty())
  {
    ...
    {
      strFilename = guih->GetLocalizeStrings().Get(20171);
    }
    else
    {
  ...
  // Root file views
    ...
    strFilename = guih->GetLocalizeStrings().Get(744);

  // Music Playlists
  else if (StringUtils::StartsWith(path, "special://musicplaylists"))
    strFilename = guih->GetLocalizeStrings().Get(136);
...
ExternalStreamInfo CUtil::GetExternalStreamDetailsFromFilename(const std::string& videoPath, const std::string& associatedFile)
{
  ...
  name += " ";
  name += guih->GetLocalizeStrings().Get(21602); // External
