...
bool CGUIScalarFeatureButton::PromptForInput(CEvent& waitEvent)
{
  ...
  switch (m_state)
  {
    ...
    {
      std::string strPrompt = CServiceBroker::GetGUI()->GetLocalizeStrings().Get(35090);  // "Press %s"
      std::string strWarn = CServiceBroker::GetGUI()->GetLocalizeStrings().Get(35091);  // "Press %s (%d)"
