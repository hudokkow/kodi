...
static int SelectBool(const std::vector<std::string>& params)
{
  ...
  pDlgSelect->Reset();
  pDlgSelect->SetHeading(CVariant{guih->GetLocalizeStrings().Get(atoi(params[0].c_str()))});
  ...
  for (unsigned int i = 1 ; i < params.size() ; i++)
  {
    if (params[i].find('|') != std::string::npos)
    {
      ...
      std::string label = guih->GetLocalizeStrings().Get(atoi(values[0].c_str()));
...
static int SetNumeric(const std::vector<std::string>& params)
{
  ...
  if (CGUIDialogNumeric::ShowAndGetNumber(value, guih->GetLocalizeStrings().Get(611)))
...
static int SetPath(const std::vector<std::string>& params)
{
  ...
  if (params.size() > 1)
  {
    ...
    {
      CMediaSource share;
      share.strName = guih->GetLocalizeStrings().Get(13278);
      share.strPath = value;
      localShares.push_back(share);
    }
  }

  if (CGUIDialogFileBrowser::ShowAndGetDirectory(localShares, guih->GetLocalizeStrings().Get(657), value))
    CSkinSettings::GetInstance().SetString(string, value);
...
static int SetFile(const std::vector<std::string>& params)
{
  ...
  else
  {
    ...
    if (CGUIDialogFileBrowser::ShowAndGetFile(localShares, strMask, guih->GetLocalizeStrings().Get(1033), value))
      CSkinSettings::GetInstance().SetString(string, value);
  }
...
static int SetImage(const std::vector<std::string>& params)
{
  ...
  if (CGUIDialogFileBrowser::ShowAndGetImage(localShares, guih->GetLocalizeStrings().Get(1030), value))
    CSkinSettings::GetInstance().SetString(string, value);
...
static int SetString(const std::vector<std::string>& params)
{
  ...
  std::string value = CSkinSettings::GetInstance().GetString(string);
  if (CGUIKeyboardFactory::ShowAndGetInput(value, CVariant{guih->GetLocalizeStrings().Get(1029)}, true))
    CSkinSettings::GetInstance().SetString(string, value);
