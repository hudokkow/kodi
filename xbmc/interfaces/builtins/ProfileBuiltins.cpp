...
static int MasterMode(const std::vector<std::string>& params)
{
  if (g_passwordManager.bMasterUser)
  {
    ...
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Warning, guih->GetLocalizeStrings().Get(20052),guih->GetLocalizeStrings().Get(20053));
  }
  else if (g_passwordManager.IsMasterLockUnlocked(true))
  {
    g_passwordManager.LockSources(false);
    g_passwordManager.bMasterUser = true;
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Warning, guih->GetLocalizeStrings().Get(20052),guih->GetLocalizeStrings().Get(20054));
  }
