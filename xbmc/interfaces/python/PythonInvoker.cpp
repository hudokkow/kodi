...
void CPythonInvoker::onError(const std::string &exceptionType /* = "" */, const std::string &exceptionValue /* = "" */, const std::string &exceptionTraceback /* = "" */)
{
  ...
  if (pDlgToast != NULL)
  {
    ...
    if (m_addon && !m_addon->Name().empty())
      message = StringUtils::Format(guih->GetLocalizeStrings().Get(2102).c_str(), m_addon->Name().c_str());
    else if (m_sourceFile == CSpecialProtocol::TranslatePath("special://profile/autoexec.py"))
      message = StringUtils::Format(guih->GetLocalizeStrings().Get(2102).c_str(), "autoexec.py");
    else
       message = guih->GetLocalizeStrings().Get(2103);
    pDlgToast->QueueNotification(CGUIDialogKaiToast::Error, message, guih->GetLocalizeStrings().Get(2104));
  }
}
