...
bool CPeripheralCecAdapter::InitialiseFeature(const PeripheralFeature feature)
{
  if (feature == FEATURE_CEC && !m_bStarted && GetSettingBool("enabled"))
  {
    ...
    if (m_configuration.serverVersion < CEC_LIB_SUPPORTED_VERSION)
    {
      ...
      CLog::Log(LOGERROR, guih->GetLocalizeStrings().Get(36040).c_str(), m_cecAdapter ? m_configuration.serverVersion : -1, CEC_LIB_SUPPORTED_VERSION);

      // display warning: incompatible libCEC
      std::string strMessage = StringUtils::Format(guih->GetLocalizeStrings().Get(36040).c_str(), m_cecAdapter ? m_configuration.serverVersion : -1, CEC_LIB_SUPPORTED_VERSION);
      CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Error, guih->GetLocalizeStrings().Get(36000), strMessage);
      m_bError = true;
      if (m_cecAdapter)
        CECDestroy(m_cecAdapter);
      m_cecAdapter = NULL;
...
bool CPeripheralCecAdapter::OpenConnection(void)
{
  ...
  std::string strMessage = StringUtils::Format(guih->GetLocalizeStrings().Get(21336).c_str(), guih->GetLocalizeStrings().Get(36000).c_str());
  CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, guih->GetLocalizeStrings().Get(36000), strMessage);
  ...
  while (!m_bStop && !bIsOpen)
  {
    if ((bIsOpen = m_cecAdapter->Open(m_strComPort.c_str(), 10000)) == false)
    {
      ...
      if (!bConnectionFailedDisplayed)
        CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Error, guih->GetLocalizeStrings().Get(36000), guih->GetLocalizeStrings().Get(36012));
...
void CPeripheralCecAdapter::CecAlert(void *cbParam, const libcec_alert alert, const libcec_parameter data)
{
  ...
  if (iAlertString)
  {
    std::string strLog(guih->GetLocalizeStrings().Get(iAlertString));
    if (data.paramType == CEC_PARAMETER_TYPE_STRING && data.paramData)
      strLog += StringUtils::Format(" - %s", (const char *)data.paramData);
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, guih->GetLocalizeStrings().Get(36000), strLog);
  }
...
bool CPeripheralCecAdapterUpdateThread::SetInitialConfiguration(void)
{
  ...
  strNotification = StringUtils::Format("%s: %s", guih->GetLocalizeStrings().Get(36016).c_str(), tvName.c_str());
  ...
  CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, guih->GetLocalizeStrings().Get(36000), strNotification);
...
void CPeripheralCecAdapterUpdateThread::Process(void)
{
  ...
  while (!m_bStop)
  {
    ...
    if (bUpdate || m_event.WaitMSec(500))
    {
      ...
      CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, guih->GetLocalizeStrings().Get(36000), guih->GetLocalizeStrings().Get(bConfigSet ? 36023 : 36024));
