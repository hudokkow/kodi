...
void CPeripherals::CreatePeripheral(CPeripheralBus &bus, const PeripheralScanResult& result)
{
  ...
  case PERIPHERAL_CEC:
    ...
    if (!m_bMissingLibCecWarningDisplayed)
    {
      ...
      CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Warning, guih->GetLocalizeStrings().Get(36000), guih->GetLocalizeStrings().Get(36017));
    }
#endif
    break;
...
void CPeripherals::OnDeviceAdded(const CPeripheralBus &bus, const CPeripheral &peripheral)
{
  ...
  if (bNotify)
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, guih->GetLocalizeStrings().Get(35005), peripheral.DeviceName());
#endif
}
...
void CPeripherals::OnDeviceDeleted(const CPeripheralBus &bus, const CPeripheral &peripheral)
{
  ...
  if (bNotify)
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, guih->GetLocalizeStrings().Get(35006), peripheral.DeviceName());
#endif
}
