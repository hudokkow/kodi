...
std::string CDirectoryNodeSeasons::GetLocalizedName() const
{
  ...
  {
  case 0:
    return guih->GetLocalizeStrings().Get(20381); // Specials
  case -1:
    return guih->GetLocalizeStrings().Get(20366); // All Seasons
  case -2:
  {
  ...
  default:
    std::string season = StringUtils::Format(guih->GetLocalizeStrings().Get(20358).c_str(), GetID()); // Season <season>
    return season;
  }
}
