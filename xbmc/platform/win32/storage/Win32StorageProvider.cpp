...
std::vector<std::string > CWin32StorageProvider::GetDiskUsage()
{
  ...
  if( dwStrLength != 0 )
  {
    ...
    do
    {
      ...
      {
        strRet = KODI::PLATFORM::WINDOWS::FromW(StringUtils::Format(L"%s %d MB %s",strDrive.c_str(), int(ULTotalFree.QuadPart/(1024*1024)),guih->GetLocalizeStrings().Get(160).c_str()));
...
bool CDetectDisc::DoWork()
{
  ...
  if(g_mediaManager.IsAudio(share.strPath))
    ...
    share.strStatus = guih->GetLocalizeStrings().Get(446);
  share.strName = share.strPath;
  share.m_ignore = true;
