...
std::vector<std::string> CStorageProvider::GetDiskUsage()
{
  ...
  strRet = FromW(StringUtils::Format(L"%s: %d MB %s", guih->GetLocalizeStrings().Get(21440), (ULTotalFree.QuadPart / (1024 * 1024)), guih->GetLocalizeStrings().Get(160).c_str()));
  ...
  for (; drivesBits > 0; drivesBits >>= 1, driveLetter++)
  {
    ...
    {
      strRet = StringUtils::Format("%s %d MB %s", strDrive.c_str(), int(ULTotalFree.QuadPart / (1024 * 1024)), guih->GetLocalizeStrings().Get(160).c_str());
      result.push_back(strRet);
    }
  }
  return result;
...
void CStorageProvider::GetDrivesByType(VECSOURCES & localDrives, Drive_Types eDriveType, bool bonlywithmedia)
{
  ...
  for (; drivesBits > 0; drivesBits >>= 1, driveLetter++)
  {
    ...
    else
    {
      ...
      {
      case DRIVE_CDROM:
        share.strName = StringUtils::Format("%s (%s)", share.strPath.c_str(), guih->GetLocalizeStrings().Get(218).c_str());
        break;
      case DRIVE_REMOVABLE:
        if (share.strName.empty())
          share.strName = StringUtils::Format("%s (%s)", guih->GetLocalizeStrings().Get(437).c_str(), share.strPath.c_str());
        break;
      default:
        if (share.strName.empty())
          share.strName = share.strPath;
