...
int CCPUInfo::GetCPUUsedPercentage()
{
  ...
  if(userTicks + niceTicks + systemTicks + idleTicks + ioTicks == 0)
    return m_lastUsedPercentage;
  result = static_cast<int>(static_cast<double>(userTicks + niceTicks + systemTicks) * 100.0 / static_cast<double>(userTicks + niceTicks + systemTicks + idleTicks + ioTicks) + 0.5);
                            ...
                            unsigned long long& system, unsigned long long& idle, unsigned long long& io)
{
  ...
  if (sysctlbyname("kern.cp_times", nullptr, &len, nullptr, 0) != 0)
    return false;
  cptimes = reinterpret_cast<long*>(malloc(len));
  if (cptimes == nullptr)
    return false;
  ...
  for (i = 0; i < m_cpuCount; i++)
  {
    ...
    if (iter != m_cores.end())
    {
      ...
      total = static_cast<double>(coreUser + coreNice + coreSystem + coreIdle + coreIO);
      if(total != 0.0f)
        iter->second.m_fPct = (static_cast<double>(coreUser + coreNice + coreSystem) * 100.0) / total;
