...
void CPowerManager::OnLowBattery()
{
  ...
  CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Warning, guih->GetLocalizeStrings().Get(13050), "");
...
{
  if (CServiceBroker::GetPowerManager().CanPowerdown())
    list.push_back(make_pair(guih->GetLocalizeStrings().Get(13005), POWERSTATE_SHUTDOWN));
  if (CServiceBroker::GetPowerManager().CanHibernate())
    list.push_back(make_pair(guih->GetLocalizeStrings().Get(13010), POWERSTATE_HIBERNATE));
  if (CServiceBroker::GetPowerManager().CanSuspend())
    list.push_back(make_pair(guih->GetLocalizeStrings().Get(13011), POWERSTATE_SUSPEND));
  if (!g_application.IsStandAlone())
  {
    list.push_back(make_pair(guih->GetLocalizeStrings().Get(13009), POWERSTATE_QUIT));
#if !defined(TARGET_DARWIN_IOS)
    list.push_back(make_pair(guih->GetLocalizeStrings().Get(13014), POWERSTATE_MINIMIZE));
#endif
  }
}
