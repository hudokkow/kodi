...
bool CSettingsManager::Load(const TiXmlElement *root, bool &updated, bool triggerEvents /* = true */, std::map<std::string, CSetting*> *loadedSettings /* = nullptr */)
{
...
void CSettingsManager::SetInitialized()
{
  ...
  {
    if (itSettingDep->second.setting == nullptr)
      continue;
...
bool CSettingsManager::GetBool(const std::string &id) const
{
  ...
  if (setting == nullptr || setting->GetType() != SettingTypeBool)
    return false;
...
double CSettingsManager::GetNumber(const std::string &id) const
{
  CSharedLock lock(m_settingsCritical);
  CSetting *setting = GetSetting(id);
  if (setting == nullptr || setting->GetType() != SettingTypeNumber)
    return 0.0;
...
bool CSettingsManager::Deserialize(const TiXmlNode *node, bool &updated, std::map<std::string, CSetting*> *loadedSettings /* = nullptr */)
{
  updated = false;
...
CSetting* CSettingsManager::CreateSetting(const std::string &settingType, const std::string &settingId, CSettingsManager *settingsManager /* = nullptr */) const
{
...
{
  if (node == nullptr || setting == nullptr || update.GetType() == SettingUpdateTypeNone)
    return false;
