...
void CDisplaySettings::OnSettingAction(std::shared_ptr<const CSetting> setting)
{
  ...
  if (settingId == "videoscreen.cms3dlut")
  {
    ...
    if (CGUIDialogFileBrowser::ShowAndGetFile(shares, ".3dlut", guih->GetLocalizeStrings().Get(36580), path))
    {
  ...
  else if (settingId == "videoscreen.displayprofile")
  {
    ...
    VECSOURCES shares;
    g_mediaManager.GetLocalDrives(shares);
    if (CGUIDialogFileBrowser::ShowAndGetFile(shares, ".icc|.icm", guih->GetLocalizeStrings().Get(36581), path))
    {
      std::static_pointer_cast<CSettingString>(std::const_pointer_cast<CSetting>(setting))->SetValue(path);
    }
  }
...
void CDisplaySettings::SettingOptionsRefreshChangeDelaysFiller(SettingConstPtr setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  for (int i = 1; i <= MAX_REFRESH_CHANGE_DELAY; i++)
    list.push_back(std::make_pair(StringUtils::Format(guih->GetLocalizeStrings().Get(13553).c_str(), (double)i / 10.0), i));
}
...
void CDisplaySettings::SettingOptionsRefreshRatesFiller(SettingConstPtr setting, std::vector< std::pair<std::string, std::string> > &list, std::string &current, void *data)
{
  ...
  {
    current = "WINDOW";
    list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(242), current));
    return;
  }
...
void CDisplaySettings::SettingOptionsResolutionsFiller(SettingConstPtr setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  {
    current = res;
    list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(242), res));
  }
  else
  {
    std::map<RESOLUTION, RESOLUTION_INFO> resolutionInfos;
...
void CDisplaySettings::SettingOptionsDispModeFiller(SettingConstPtr setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  if (g_advancedSettings.m_canWindowed && CServiceBroker::GetWinSystem()->CanDoWindowed())
    list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(242), DM_WINDOWED));

  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(244), DM_FULLSCREEN));
}
...
{
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36580), CMS_MODE_3DLUT));
#ifdef HAVE_LCMS2
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36581), CMS_MODE_PROFILE));
#endif
}
...
{
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36586), CMS_WHITEPOINT_D65));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36587), CMS_WHITEPOINT_D93));
}
...
{
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36588), CMS_PRIMARIES_AUTO));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36589), CMS_PRIMARIES_BT709));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36579), CMS_PRIMARIES_BT2020));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36590), CMS_PRIMARIES_170M));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36591), CMS_PRIMARIES_BT470M));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36592), CMS_PRIMARIES_BT470BG));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36593), CMS_PRIMARIES_240M));
}
...
{
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36582), CMS_TRC_BT1886));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36583), CMS_TRC_INPUT_OFFSET));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36584), CMS_TRC_OUTPUT_OFFSET));
  list.push_back(std::make_pair(guih->GetLocalizeStrings().Get(36585), CMS_TRC_ABSOLUTE));
}
