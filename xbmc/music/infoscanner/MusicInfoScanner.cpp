...
void CMusicInfoScanner::Process()
{
  ...
  try
  {
    if (m_showDialog && !CServiceBroker::GetSettings().GetBool(CSettings::SETTING_MUSICLIBRARY_BACKGROUNDUPDATE))
    {
      ...
      if (dialog)
        m_handle = dialog->GetHandle(guih->GetLocalizeStrings().Get(314));
    }
    ...
    if (m_scanType == 0) // load info from files
    {
      ...
      if (m_handle)
        m_handle->SetTitle(guih->GetLocalizeStrings().Get(505));
      ...
      if (commit)
      {
        ...
        if (m_needsCleanup)
        {
          if (m_handle)
          {
            m_handle->SetTitle(guih->GetLocalizeStrings().Get(700));
            m_handle->SetText("");
          }
          ...
          if (m_handle)
            m_handle->SetTitle(guih->GetLocalizeStrings().Get(331));
...
bool CMusicInfoScanner::DoScan(const std::string& strDirectory)
{
  if (m_handle)
  {
    m_handle->SetTitle(guih->GetLocalizeStrings().Get(506)); //"Checking media files..."
  ...
  { // path has changed - rescan
    ...
    if (m_handle)
      m_handle->SetTitle(guih->GetLocalizeStrings().Get(505)); //"Loading media information from files..."
...
void CMusicInfoScanner::FileItemsToAlbums(CFileItemList& items, VECALBUMS& albums, MAPSONGS* songsMap /* = NULL */)
{
  ...
  for (std::map<std::string, VECSONGS>::iterator songsByAlbumName = songsByAlbumNames.begin(); songsByAlbumName != songsByAlbumNames.end(); ++songsByAlbumName)
  {
    ...
    */
    std::string various = guih->GetLocalizeStrings().Get(340); // Various Artists
  ...
  while (!stop)
  {
    ...
    if (albumDownloadStatus == INFO_NOT_FOUND)
    {
      if (pDialog && bAllowSelection)
      {
        ...
        if (!CGUIKeyboardFactory::ShowAndGetInput(strTempAlbum, CVariant{ guih->GetLocalizeStrings().Get(16011) }, false))
          albumDownloadStatus = INFO_CANCELLED;
        else
        {
          std::string strTempArtist(album.GetAlbumArtistString());
          if (!CGUIKeyboardFactory::ShowAndGetInput(strTempArtist, CVariant{ guih->GetLocalizeStrings().Get(16025) }, false))
            albumDownloadStatus = INFO_CANCELLED;
          else
          {
      ...
      else
      {
        CServiceBroker::GetEventLog().Add(EventPtr(new CMediaLibraryEvent(
          ...
          StringUtils::Format(guih->GetLocalizeStrings().Get(24147).c_str(), MediaTypeAlbum, album.strAlbum.c_str()),
                                            ...
                                            CGUIDialogProgress* pDialog /* = NULL */)
{
  ...
  while (!stop)
  {
    ...
    if (artistDownloadStatus == INFO_NOT_FOUND)
    {
      if (pDialog && bAllowSelection)
      {
        if (!CGUIKeyboardFactory::ShowAndGetInput(artist.strArtist, CVariant{ guih->GetLocalizeStrings().Get(16025) }, false))
      ...
      else
      {
        CServiceBroker::GetEventLog().Add(EventPtr(new CMediaLibraryEvent(
          ...
          StringUtils::Format(guih->GetLocalizeStrings().Get(24147).c_str(), MediaTypeArtist, artist.strArtist.c_str()),
                                     ...
                                     CGUIDialogProgress* pDialog)
{
  if (m_handle)
  {
    m_handle->SetTitle(StringUtils::Format(guih->GetLocalizeStrings().Get(20321).c_str(), info->Name().c_str()));
    m_handle->SetText(album.GetAlbumArtistString() + " - " + album.strAlbum);
  }
      ...
      if (scraper.GetAlbumCount() > 1) // score the matches
      {
        ...
        if (pDialog && bestRelevance < THRESHOLD)
        {
          ...
          { // none chosen
            ...
            if (!CGUIKeyboardFactory::ShowAndGetInput(strNewAlbum, CVariant{guih->GetLocalizeStrings().Get(16011)}, false))
              return INFO_CANCELLED;
            if (strNewAlbum == "")
              return INFO_CANCELLED;

            std::string strNewArtist = album.GetAlbumArtistString();
            if (!CGUIKeyboardFactory::ShowAndGetInput(strNewArtist, CVariant{guih->GetLocalizeStrings().Get(16025)}, false))
              return INFO_CANCELLED;
                                      ...
                                      CGUIDialogProgress* pDialog)
{
  if (m_handle)
  {
    m_handle->SetTitle(StringUtils::Format(guih->GetLocalizeStrings().Get(20320).c_str(), info->Name().c_str()));
    m_handle->SetText(artist.strArtist);
  }
  ...
  if (result == CInfoScanner::NO_NFO && !bMusicBrainz)
  {
    if (scraper.GetArtistCount() >= 1)
    {
      ...
      if (pDialog && scraper.GetArtistCount() > 1)
      {
        ...
        if (pDlg)
        {
          pDlg->SetHeading(CVariant{guih->GetLocalizeStrings().Get(21890)});
          pDlg->Reset();
          pDlg->EnableButton(true, 413); // manual
