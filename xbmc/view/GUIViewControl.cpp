...
void CGUIViewControl::UpdateViewAsControl(const std::string &viewLabel)
{
  ...
  {
    IGUIContainer *view = static_cast<IGUIContainer*>(m_visibleViews[i]);
    std::string label = StringUtils::Format(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(534).c_str(), view->GetLabel().c_str()); // View: %s
  ...
  std::string label = StringUtils::Format(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(534).c_str(), viewLabel.c_str()); // View: %s
