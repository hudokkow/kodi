...
{
  for (unsigned int i = 0; i < NUM_FIELDS; i++)
    if (field == fields[i].field) return guih->GetLocalizeStrings().Get(fields[i].localizedString);
  return guih->GetLocalizeStrings().Get(16018);
}
...
std::string CSmartPlaylistRule::GetLocalizedGroup(Field group)
{
  ...
  {
    if (group == groups[i].field)
      return guih->GetLocalizeStrings().Get(groups[i].localizedString);
  }

  return guih->GetLocalizeStrings().Get(groups[0].localizedString);
}
