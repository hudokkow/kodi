...
bool CHTTPWebinterfaceHandler::ResolveAddon(const std::string &url, ADDON::AddonPtr &addon, std::string &addonPath)
{
  ...
  if (url.find("/addons/") == 0 && url.size() > 8)
  {
    ...
    if (!ADDON::CAddonMgr::GetInstance().GetAddon(components.at(1), addon) || addon == nullptr)
      return false;
  ...
  else if (!ADDON::CAddonMgr::GetInstance().GetDefault(ADDON::ADDON_WEB_INTERFACE, addon) || addon == nullptr)
    return false;
