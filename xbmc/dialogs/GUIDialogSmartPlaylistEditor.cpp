...
void CGUIDialogSmartPlaylistEditor::OnOK()
{
  ...
  if (m_path.empty())
  {
    ...
    std::string path;
    if (CGUIKeyboardFactory::ShowAndGetInput(filename, CVariant{CServiceBroker::GetGUI()->GetLocalizeStrings().Get(16013)}, false))
    {
...
void CGUIDialogSmartPlaylistEditor::OnLimit()
{
  ...
  for (auto limit = limits.begin(); limit != limits.end(); limit++)
  {
    ...
    if (*limit == 0)
      dialog->Add(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21428));
    else
      dialog->Add(StringUtils::Format(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21436).c_str(), *limit));
  }
...
void CGUIDialogSmartPlaylistEditor::OnOrder()
{
  ...
  for (auto order: orders)
    dialog->Add(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(SortUtils::GetSortLabel(order)));
  dialog->SetHeading(CVariant{ 21429 });
  dialog->SetSelected(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(SortUtils::GetSortLabel(m_playlist.m_orderField)));
  dialog->Open();
  int newSelected = dialog->GetSelectedItem();
...
void CGUIDialogSmartPlaylistEditor::UpdateButtons()
{
  ...
  {
    SET_CONTROL_LABEL2(CONTROL_NAME, CServiceBroker::GetGUI()->GetLocalizeStrings().Get(16035));
  ...
  if (m_playlist.m_ruleCombination.GetType() == CSmartPlaylistRuleCombination::CombinationOr)
    SET_CONTROL_LABEL2(CONTROL_MATCH, CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21426)); // one or more of the rules
  else
    SET_CONTROL_LABEL2(CONTROL_MATCH, CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21425)); // all of the rules
  CONTROL_ENABLE_ON_CONDITION(CONTROL_MATCH, m_playlist.m_ruleCombination.m_rules.size() > 1);
  if (m_playlist.m_limit == 0)
    SET_CONTROL_LABEL2(CONTROL_LIMIT, CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21428)); // no limit
  else
    SET_CONTROL_LABEL2(CONTROL_LIMIT, StringUtils::Format(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21436).c_str(), m_playlist.m_limit));
  ...
  CFileItemPtr item(new CFileItem("", false));
  item->SetLabel(CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21423));
  m_ruleLabels->Add(item);
  ...
  if (m_playlist.m_orderDirection != SortOrderDescending)
  {
    SET_CONTROL_LABEL2(CONTROL_ORDER_DIRECTION, CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21430));
  }
  else
  {
    SET_CONTROL_LABEL2(CONTROL_ORDER_DIRECTION, CServiceBroker::GetGUI()->GetLocalizeStrings().Get(21431));
  }

  SET_CONTROL_LABEL2(CONTROL_ORDER_FIELD, CServiceBroker::GetGUI()->GetLocalizeStrings().Get(SortUtils::GetSortLabel(m_playlist.m_orderField)));
...
std::string CGUIDialogSmartPlaylistEditor::GetLocalizedType(PLAYLIST_TYPE type)
{
  for (unsigned int i = 0; i < NUM_TYPES; i++)
    if (types[i].type == type)
      return CServiceBroker::GetGUI()->GetLocalizeStrings().Get(types[i].localizedString);
  assert(false);
  return "";
}
